...
void markerListenerCB(const geometry_msgs::PointStamped& p_msg) {
    marker.points.push_back(p_msg.point);
    marker.header = p_msg.header;  // this can overwrite the reference frame;
    ROS_INFO("got a new point! x,y,z = %f %f %f",p_msg.point.x,p_msg.point.y,p_msg.point.z);
