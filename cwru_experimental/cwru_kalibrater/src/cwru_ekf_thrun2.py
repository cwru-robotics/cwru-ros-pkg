...
class EKF:    
  ...
  def prediction(self, msg):
    ...
    if(self.firstRun == 1):
      ...
      wy = (msg.yaw_rate/1000.0)/self.yc*3.1415/180.0+Vb
      
      self.x[0] = x + venc*self.dt*cos(th)
      self.x[1] = y + venc*self.dt*sin(th)
      self.x[2] = th + wy*self.dt
      self.x[3] = venc
      self.x[4] = wy
  ...
  def modelJacobian(self, msg):
    ...
    if(self.firstRun == 1):
      ...
      self.A = eye(6)
      self.A[0,2] = -venc*dt*sin(th)
      self.A[0,3] = dt*cos(th)
      self.A[1,2] = venc*dt*cos(th)
  ...
  def sensor_update(self, msg):
    ...
    if(self.firstRun == 1):
      ...
      '''yawVar = .1#self.variance_from_measurement(yv, self.yawV[0], self.yawV[1], self.yawV[2])
