...
class EKF:    
  ...
  def sensor_update(self, msg):
    ...
    if(self.firstRun == 1):
      ...
      R = diagflat(array([lev, rev, yawVar]))
      
      y = z - self.sensor_prediction()
      
      #print "w: %.3f enc: %.3f yaw: %.3f yawp: %.3f b: %.3f" % (self.x[4], wenc, yv, self.x[4]+self.x[5], self.x[5])
  ...
  def sensorJacobian(self):
    ...
    self.H[0,3] = dt
    self.H[1,3] = dt
    self.H[0,4] = self.track*dt/2.0
    self.H[1,4] = -self.track*dt/2.0
    self.H[2,4] = 1.0
    self.H[2,5] = 1.0
